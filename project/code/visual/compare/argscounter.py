...
if __name__ == '__main__':
    ...
    model = YOLO(r'D:\Workspace\YOLO\models\ultralytics\runs\train\exp11\weights\best.pt') # select your model.pt path
    # model = RTDETR(r'ultralytics/cfg/models/rt-detr/rtdetr-resnet50.yaml').model
    model.to('cuda')
    model.fuse()
    input = torch.randn(batch_size, 3, height, width).to('cuda')
    total_flops, total_params, layers = profile(model.model, inputs=(input,), verbose=True, ret_layer_info=True)
